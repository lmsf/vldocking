...
class RelativeDockingUtilities {
	...
	private static SplitContainer findBestContainer(Container ancestorContainer, RelativeDockablePosition position) {
		...
		// and ensure this split is linked to ancestorContainer by a hierarchy
		// of split containers
		...
		while(splitComp != null && splitComp != ancestorContainer && ! found) {
			...
			if(splitComp instanceof SplitContainer) {
				// ok we've found a split : now let's check if there's a direct
				// split hierarchy
		...
		if(splitComp instanceof SplitContainer) {
			// try to enhance the selection when there are global anchors
			// //2005/10/10
			// we're now looking for a split container whose anchors would
			// better match
	...
	private static SplitContainer findBetterContainer(SplitContainer split, Container dockingPanel, boolean anchoredTop, boolean anchoredLeft, boolean anchoredBottom, boolean anchoredRight) {
		// we already have found a splitContainer, but it can be subOptimal in
		// some cases
		// for instance when we want to anchor a component at the bottom of the
		// container (full width),
		// and there are two horizontal splits there, we might return the
		// internal split (not full width),
		// and not the external (full width) one.
		// this method will try to find a splitcontainer upper in the hierarchy,
		// with better anchors
		/*
			...
			if(comp instanceof SplitContainer) {
				...
				if(sc.getOrientation() == JSplitPane.VERTICAL_SPLIT) {
					if(sc.getTopComponent() == child) {
						contact = contact & (~ AnchorConstraints.ANCHOR_BOTTOM); // not
																					// at
																					// bottom
					} else {
						contact = contact & (~ AnchorConstraints.ANCHOR_TOP); // not
																				// on
																				// top
					}
				} else {
					if(sc.getLeftComponent() == child) {
						contact = contact & (~ AnchorConstraints.ANCHOR_RIGHT); // not
																				// right
					} else {
						contact = contact & (~ AnchorConstraints.ANCHOR_LEFT); // not
																				// left
					}
				}
			}
	...
	public static SingleDockableContainer insertDockable(Container relativeAncestorContainer, Dockable dockable, RelativeDockablePosition position) {
		...
		if(relativeAncestorContainer.getComponentCount() == 0) { // empty
																	// docking
																	// panel
			...
			if(split != null) {
				...
				// heuristics : try to find an anchor, and which
				...
				// System.out.println("anchors : " + anchoredTop + ", "+
				// anchoredLeft + ", "
				...
				// float splitHeightRatio = Math.abs(1 - split.getHeight() /
				// (float)h);
				...
				if(split.getOrientation() == JSplitPane.VERTICAL_SPLIT) {
					...
					// proportions used when splitting a vertical split
					// vertically
					...
					if(anchoredTop) {
						if(anchoredLeft) {
							if(anchoredBottom) { // TLB = full left
								...
								if(splitWidthRatio < 0.1f && split.getHeight() > h) {// 2006/09/12
									// almost the same width, but the split is
									// taller : we split it
									// vertically
									SplitContainer newSplit = new SplitContainer(JSplitPane.VERTICAL_SPLIT);
									DockingUtilities.replaceChild(split.getParent(), split, newSplit);
						...
						if(anchoredRight) { // 2005/10/10
							// left + right on a vertical split, means we have
						...
						// todo : check how it's done now for the split-h part
					...
					// horizontal split
					// proportions used when splitting a horizontal split
					// horizontally
					...
					if(anchoredTop) {
						if(anchoredLeft) {
							if(anchoredBottom) { // TLB = full left
								...
								if(splitWidthRatio < 0.1f && split.getHeight() > h) { // 2006/09/12
									...
									// otherwise we just split vertically on the
									// left side
									SplitContainer newSplit = new SplitContainer(JSplitPane.VERTICAL_SPLIT);
									DockingUtilities.replaceChild(split, left, newSplit);
									newSplit.setTopComponent((Component) dockableContainer);
							...
							if(anchoredBottom) { // top + right + bottom == full
													// right
								SplitContainer newSplit = new SplitContainer(JSplitPane.HORIZONTAL_SPLIT);
							...
							// as we are inserting a vertical element into a
							// horizontal split
						...
						if(anchoredRight) {
							...
							DockingUtilities.replaceChild(split.getParent(), split, newSplit);
							newSplit.setTopComponent((Component) dockableContainer);
							newSplit.setBottomComponent(split); // todo : check
																// this case :
																// why is the
																// new component
																// on top
						...
						if(splitWidthRatio < 0.1f && split.getHeight() > h) { // 2006/09/12
							// almost the same width, but the split is taller :
							// we split it
							// vertically
							SplitContainer newSplit = new SplitContainer(JSplitPane.VERTICAL_SPLIT);
							DockingUtilities.replaceChild(split.getParent(), split, newSplit);
						...
						// - we can guess if the component was horizontaly
						// splitted or vertically,
				...
				if(anchoredTop) {
					if(anchoredLeft) {
						...
						if(anchoredBottom) { // top + right + bottom == on the
												// right
					...
					} else if(anchoredRight) { // left + right, but not
												// top/bottom...
						if(centerY < relativeAncestorContainer.getHeight() / 2) { // center
																					// is
																					// upper
																					// part
							vSplitAndResize(singleComp, (Component) dockableContainer, singleComp, proportionH);
		...
		// DockingActionEvent action =
		// getInsertionDockingAction(relativeAncestorContainer, dockable,
		// position);

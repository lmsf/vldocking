...
public class DockingUtilities {
	...
	public static TabbedDockableContainer findTabbedDockableContainer(Dockable dockable) {
		 ...
		 */
		while(comp != null) {
			// new version to support compoundDockable : stop after the first
			// parent of
	...
	public static void replaceChild(Container parent, Component child, Component newChild) {
		// System.out.println("replace child : PARENT=" + parent + ", CHILD=" +
		// child + ", NEW=" + newChild);
		if(parent instanceof SplitContainer) {
			...
			// the newChild can be a dummy JComponent (when used in
			// maximize/restore)
			// or a fully featured dockable (other usages)
			// this is not really clean : it's the problem when using interfaces
			// over subclasses...

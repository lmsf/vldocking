...
/**
 * This class is the interface between a JTabbedPane and a smart icon. <p> As
 * JTabbedPanes cannot use any JComponents as tab selectors (the access if
 * protected and we just have a label, an icon and a tooltip), we have to rely on
 * tricks to bypass them. <p> This class is used (with a mouse listener) to
 * forward events into the smart icon. which is responsible for faking an
 * enhanced tab selector (including label and optional buttons).
 * 
 * 
...
public class JTabbedPaneSmartIconManager implements MouseListener, MouseMotionListener {
	...
	/**
	 * MouseListener implementation, use to track mouse behaviour inside the tab
	 * selector bounds and forward them to the appropriate smart icon.
	 */
		...
		if(pressedIcon != null) {
			...
			try {
				if(pressedIcon.onMouseReleased(eSmart)) {
					tabbedPane.repaint(r.x, r.y, r.width, r.height); // no
																		// choice
																		// but
																		// trigger
																		// a
			...
			} catch(Exception ignore) { // bug database : 5075526 this is to
	...
	public void mousePressed(MouseEvent e) {
		...
		if(targetTab != - 1) {
			...
			if(icon instanceof JTabbedPaneSmartIcon) {
				...
				// as the icon is the only thing visible, we consider it
				// centered in the tab
				// (which is the default paint behaviour of BasicTabbedPaneUI,
				// and should be okay
	...
	/**
	 * verify if the tab count hasn't changed. <p> There is always the risk that
	 * the last tab has been removed (and movedTab will be out of bounds)
	 * 
	 */
	private void checkTabCount() {
		if(movedTab >= tabbedPane.getTabCount()) { // too late : tab has been
													// removed
			movedTab = - 1;
			movedIcon = null;
		}
			...
			if(icon instanceof JTabbedPaneSmartIcon) {
				...
				if(movedIcon != null && movedIcon != smartIcon) {
					...
					if(movedIcon.onMouseExited(eSmart)) {
						...
						// tabbedPane.repaint(prevRect.x, prevRect.y,

...
public class ToolBarPanelBorder implements Border {
	...
	/* Updated to work for the new image position. */
	private static Image borderImage = new ImageIcon(ClassLoader.getSystemResource("toolbarpanelborder.png")).getImage();
	...
	public void paintBorder(Component component, Graphics graphics, int x, int y, int w, int h) {
		...
		switch(panelPosition) {
			case TOP_PANEL:
				...
				// bottom-right corner
				/*
				 * graphics.drawImage(borderImage, x+ w -SIDE, y+h-SIDE, x+w,
				 * y+h, borderWidth-SIDE,borderHeight-SIDE, borderWidth,
				 * borderWidth, null); // bottom left
				 * graphics.drawImage(borderImage, x, y+h-SIDE, x+SIDE, y+h, 0,
				 * borderHeight-SIDE, SIDE, borderHeight, null);
				 */
				break;
			case LEFT_PANEL:
				// vertical right
		...
		/*
		 * // top right corner graphics.drawImage(borderImage, x+ w -5, y, x+w,
		 * y+5, borderWidth-5,0, borderWidth, 5, null); // vertical right
		 * graphics.drawImage(borderImage, x+ w -5, y+5, x+w, y+h-5,
		 * borderWidth-5,5, borderWidth, borderHeight-5, null); // bottom-right
		 * corner graphics.drawImage(borderImage, x+ w -5, y+h-5, x+w, y+h,
		 * borderHeight-5,borderWidth-5, borderWidth, borderHeight, null); //
		 * horizontal bottom graphics.drawImage(borderImage, x+5, y+h-5, x+w-5,
		 * y+h, 5,borderHeight-5, borderWidth-5, borderHeight, null); // bottom
		 * left corner graphics.drawImage(borderImage, x, y+h-5, x+5, y+h, 0,
		 * borderHeight-5, 5, borderHeight, null); // horizontal top
		 * graphics.drawImage(borderImage, x+5, y, x+w-5, y+5, 5,0,
		 * borderHeight-5, 5, null); // vertical left
		 * graphics.drawImage(borderImage, x, y+5, x+5, y+h-5, 0,5, 5,
		 * borderHeight-5, null); // top left corner
		 * graphics.drawImage(borderImage, x, y, x+5, y+5, 0,0, 5, 5, null);
		 */
	}

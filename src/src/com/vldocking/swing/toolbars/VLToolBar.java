...
/**
 * A JToolbar suitable for drag and drop purposes. <p> To use the toolbar
 * feature, you must follow this pattern : <ul> <li> The toolbars must be
 * contained initially in a ToolBarPanel (which can contain multiple toolbars)
 * <li> The ToolBarPanel must be contained in a ToolBarContainer (which can
 * contain as much as 4 ToolBarPanels , one on each border). <li> The center
 * component of the ToolBarContainer can be the DockingDesktop, or any user
 * component. </ul>
 * 
 * <p align="center"><img src="doc-files/overview.gif"><br> The VLToolBar
 * contains buttons and other components, and is included in a ToolBarPanel of a
 * ToolBarContainer . </p> <p> Loading and saving toolbar configurations is done
 * with the {@link ToolBarIO} utility class. <p> Toolbars can now be collapsed
 * (to gain space on screen). this property is disabled by default to keep
 * compatibility with previous releases. For a better usage of collapsing it is
 * better to set a tooltip text on the toolbar (it will be propagated to the
 * gripper, which will be the only one component visible when collapsed).
 * 
 * @author Lilian Chamontin, VLSolutions
 * @update 2006/02/20 Lilian Chamontin : added support for collapsible toolbars
 * @update 2006/03/16 Lilian Chamontin : added animation effect when collapsing
 *         toolbars
 * @update 2006/12/01 Lilian Chamontin : added protection againt class cast when
 *         dragging
 */
public class VLToolBar extends JPanel {
	...
	/**
	 * Constructs a toolbar with a null name (this toolbar will not be able to
	 * write its structure in a XML encoder, unless you call setName(String)
	...
	/**
	 * Constructs a toolbar with the given name. <p> The name is used when
	 * reading/writing XML configuration. It must not be null if you use this
	 * feature.
	 */
	...
	/**
	 * Install custom UI for this button : a light rollover effet and a custom
	 * rounded/shaded border. <p> This method can be overriden to replace the
	 * provided "look and feel" which uses the follwing configuration : <ul>
	 * <li> install a VLButtonUI <li> set 2 pixels margins <li> set a
	 * ToolBarButtonBorder. </ul>
	 */
	...
	/**
	 * Used internally to add a mouse listener to the button. <p> Can be
	 * overriden to implement custom event handling.
	 */
	 ...
	 * overriden to provide another look or feel. <p> Default settings are :
	 * <ul><li> setRolloverEnabled(true) <li> setContentAreaFilled(false); <li>
	 * setOpaque(false) <li> setBorderPainted(false) </ul>
	 */
	...
	/**
	 * Updates the rolloverBorderPainted property. <p> If true, when one of the
	 * toolbar buttons is rolled-over, its border will be shown. <P>
	...
	/**
	 * Updates the rolloverContentAreaFilled property. <p> If true, when one of
	 * the toolbar buttons is rolled-over, its content will be filled. <p>
	 * Default value is <b>false</b> to accomodate with VLButtonUI which paints
	 * itself the button interiors.
	 * 
	 */
	...
	/**
	 * Updates the useCustomUI property. <p> Default value is true. <p> When set
	 * to true the installButtonUI() method will be called when a button is
	...
	/**
	 * Changes the border used during drag and drop of the toolbar. <p> For a
	 * better user experience, please note to install an empty border of the
	 * same Insets than this one (otherwise, there will be a size change of the
	 * toolbar at the beginning of the drag and after the drop).
	 */
	...
	private void gripperDragged(MouseEvent e) {
		...
		if(! (panel.getParent() instanceof ToolBarContainer)) { // 2006/12/01
			// this is a safety for users willing to include toolbar panels
			// outside
			// a toolbar container (in that case, these toolbars aren't
			// draggable).
			return;
		}
		...
		if(panel.contains(tbPanelPoint)) {
			...
			// we're outside this toolbar panel, check if we're above another
			// toolbar
		...
		if(panel != null && panel != getParent()) {
			if(panel.isVisible()) {
				...
				if(invisibleBounds.contains(containerPoint)) {
					...
					parent.remove(this);
					panel.add(this, new ToolBarConstraints(0, 0)); // the panel
																	// is not
																	// visible
																	// == empty
					panel.revalidate();
	...
	/**
	 * Updates the orientation of this toolbar. <p> This method is called
	 * directly by the framework on toolbar installation or during drag and
	 * drop.
	 */
	...
	/**
	 * Updates the tooltip of the toolbar, and propagates it to the toolbar
	 * gripper.
	 */
	...
	public void setCollapsed(final boolean collapsed) {
		if(collapsed) {
			// added a subtle animation effect to give more polish to the
			// collapsing //2006/03/16

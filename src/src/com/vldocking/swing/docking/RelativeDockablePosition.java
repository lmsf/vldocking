 ...
 * of docked components. <p> <code>x, y, w, h</code> fields are expressed with
 * <code>double</code> values varying from 0 to 1. <p> <ul> <li> x : 0 = left, 1
 * = right <li> y : 0 = top, 1 = bottom <li> w : 0 = no width, 1-x = full
 * remaining width <li> h : 0 = no height, 1-y = full remaining height </ul>
 * 
 * <p> Objects of this class are used to reposition a Dockable on the desktop,
 * based on lazy constraints : As the desktop is a mix of horizontal and
 * vertical split panes ({@link SplitContainer}) it is not always possible to
 * put a dockable exactly where you want given a set of x,y, w, and h
 * constraints.
 * 
 * 
...
public class RelativeDockablePosition {
	...
	/**
	 * convenience field referencing the CENTER part of TOP border of the
	 * DockingDesktop
	 */
	...
	/**
	 * convenience field referencing the CENTER part of the RIGHT border of the
	 * DockingDesktop
	 */
	...
	/**
	 * convenience field referencing the CENTER part of the BOTTOM border of the
	 * DockingDesktop
	 */
	...
	/**
	 * convenience field referencing the CENTER part of the LEFT border of the
	 * DockingDesktop
	 */
	public static final RelativeDockablePosition LEFT_CENTER = new RelativeDockablePosition(0, 0.4, 0.2, 0.2);

	private double x, y, w; // relative positionning (0-1) of the component in
							// the container

	/**
	 * The container used for relative positioning (can be a top level
	 * DockingPanel or an intermediate CompoundDockingPanel)
	 */
	...
	/**
	 * @see AnchorConstraints.ANCHOR_TOP (this field is an ORing of possible
	 *      anchors) and contains 4 boolean values (TOP, LEFT, BOTTOM, RIGHT)
	 */
	private int anchors;

	/**
	 * Construct a RelativeDockablePosition, assigning x, y, w, h values from
	 * the current position of a dockable in its desktop container. <p> This
	 * constructor is a convenience for the framework, and should not be used by
	 * API users (thus is package protected visibility).
	 * */
	...
	/**
	 * Constructs an empty relative position (all fields are initialized with
	 * 0.0 values)
	 */
	...
	/**
	 * Constructs a relative position with given location and size. <p> Please
	 * remember that the values must be between 0 and 1, as they designate
	...
	/**
	 * Constructs a relative position with a given location and size, relative
	 * to a container
	 */
	...
	/**
	 * Returns a meaningfull string representation of this
	...
	/**
	 * Returns the ancestor container used as a reference for the relative
	 * potionning <p> This can be a DockingPanel (top level ancestor) or
	 * CompoundDockingPanel (intermediate ancestor)
	 */
	...
	/**
	 * Returns an integer value representing the anchoring of the dockable
	 * relatively to its ancestor container. <p> For example a dockable at the
	 * top left angle of an ancestor container will have an "anchors" value or :

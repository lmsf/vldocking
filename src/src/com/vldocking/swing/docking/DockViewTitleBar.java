 ...
 * : <ul> <li> Icon <li> Name <li> Tooltip <li> Notification (blinking
 * background). <li> And some buttons if their corresponding Action contains the
 * </ul> <p> This title bar supports buttons used for docking features : <table
 * border="1"><tr><th> function </th><th>Version 1.1</th><th>Version
 * 2.0</th></tr> <tr><td>maximize</td><td><img
 * src="doc-files/maximize16.gif"></td><td><img
 * src="doc-files/maximize16v2rollover.png"></td></tr>
 * <tr><td>restore</td><td><img src="doc-files/restore16.gif"></td><td><img
 * src="doc-files/restore16v2rollover.png"></td></tr> <tr><td>hide</td><td><img
 * src="doc-files/hide16.gif"></td><td><img
 * src="doc-files/hide16v2rollover.png"></td></tr> <tr><td>dock</td><td><img
 * src="doc-files/dock16.gif"></td><td><img
 * src="doc-files/dock16v2rollover.png"></td></tr> <tr><td>close</td><td><img
 * src="doc-files/close16.gif"></td><td><img
 * src="doc-files/close16v2rollover.png"></td></tr> <tr><td>float
 * (detach)</td><td>n/a </td><td><img
 * src="doc-files/float16v2rollover.png"></td></tr> <tr><td>attach</td><td>n/a
 * </td><td><img src="doc-files/attach16v2rollover.png"></td></tr> </table>
 * 
 * <p> The buttons managed have no effect on the state of the dockable : they
 * just fire property change events, and it is the responsibility of the
 * DockableContainer to listen to those events and to relay the operation to the
 * docking desktop. <p> Note : the UI Delegate of the DockViewTitleBar is the
 * {@link com.vldocking.swing.docking.ui.DockViewTitleBarUI}
 * 
...
public class DockViewTitleBar extends JPanel implements DockableDragSource {
	...
	// this creation tricks to manage the UI class requiring access to the
	// components
	...
	private PropertyChangeListener dockKeyListener = new PropertyChangeListener() {
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			/*
			 * else if (pName.equals(DockKey.PROPERTY_DOCKABLE_STATE)){ int
			 * newState = ((Integer)e.getNewValue()).intValue(); int oldState =
			 * ((Integer)e.getOldValue()).intValue(); switch (newState){ case
			 * DockableState.HIDDEN : setAutoHide(true); break; case
			 * DockableState.MAXIMIZED : setMaximized(true); break; case
			 * DockableState.DOCKED : if (oldState == DockableState.HIDDEN){
			 * setAutoHide(false); } break; default: //@todo : managed the
			 * floatable state // nothing to track here } }
			 */
		}
	};
	...
	// private static FocusHighlighter focusHighlighter = new
	// FocusHighlighter(); //2005/11/10
	...
	/**
	 * Constructs a title bar for the specified dockable. <p> Warning : a
	 * DockViewTitleBar can be used with multiple dockables (this is the case
	 * for example in autohide borders, where a single titlebar is shared by all
	 * hidden dockables (shown only when one is expanding).
	 */
	...
	/**
	 * Notification of completion of layout. <p> This hook can be used to insert
	 * customized buttons without otherwise having to fully replace the UI
	 * delegate
	 */
	...
	/**
	 * Returns the desktop associated to this title bar, if one has been
	 * registered with #installDocking(DockingDesktop), or null.
	 */
	...
	/**
	 * Overriden as a means to unregister internal listeners, do not call
	 * directly
	 */
	public void removeNotify() {
		 ...
		 * removePropertyChangeListener( focusHighlighter); 2005/11/10: now it's
		 * shared between title bars
		 */
		if(target != null) {
	...
	/**
	 * Returns the label used to display the dockkey name. <p> Shouldn't be used
	 * to update the title : the best way is to update the DockKey (property
	...
	/**
	 * returns the button used for hiding or docking the view. <p> As hiding and
	 * docking are mutually exclusive, the same button is used for both purposes
	 */
	...
	/**
	 * returns the button used for maximizing or restoring the view. <p> As
	 * those operations are mutually exclusive, the same button is used for both
	 * purposes
	 */
	...
	private void maximizeAction() {
		...
		if(target.getDockKey().isMaximizeEnabled()) {
			...
			/*
			 * We do not use fireVetoableChange, to keep maximization processing
			 * outside the title bar (everything is driven par the
			 * dockingdesktop and the SingleDockableContainer
			 */
	...
	/**
	 * Returns true if the dockable is the currently active one. <p> There is at
	 * most one active dockable for a dekstop, and it there is one, it is the
	 * one which contains the keybord focused component.
	 * */
	...
	/**
	 * Updates the active property. A title bar is active when the dockable it
	 * is for is ancestor of the keybord focused component.
	 * */
	...
	public void setDockable(Dockable dockable) {
		...
		if(dockable != null) {
			...
			/*
			 * dockButton.setVisible(key.isAutoHideEnabled()); this.autoHide =
			 * key.getLocation() == DockableState.HIDDEN;
	...
	public void uninstallDocking(DockingDesktop desktop) {
		// System.out.println("uninstallDocking TITLE on " +
		// target.getDockKey());
		this.desktop = null;
	...
	/**
	 * notifies the source when the drag operation has ended (by a drop or
	 * cancelled)
	 * 
	 * @since 2.1.3
	 */
	...
	private static class FocusHighlighter implements PropertyChangeListener {
		...
		FocusHighlighter() {
			// this is a singleton so we register here for keyboard focus events
			// properties
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if(ancestor != null) {
				if(ancestor instanceof DockView) {
					...
					if(tb == activeTitleBar) {
						...
						if(target != null && target.getDockKey() != null) { // 2007/02/27
																			// fixed
																			// NPE
			 ...
			 * if (activeTitleBar != null){ Container parent =
			 * activeTitleBar.getParent(); if (parent != null && c != null &&
			 * parent.isAncestorOf(c)) { if (!active) { setActive(true); } //
			 * reset notification (blinking)
			 * target.getDockKey().setNotification(false); } else if (active) {
			 * setActive(false); }
			 */
		}
	}

...
abstract class AbstractDragControler implements DragControler {
	...
	public void mousePressed(MouseEvent e) {
		if(SwingUtilities.isLeftMouseButton(e)) {
			...
			if(dragPoint != null) { // we were dragging
				...
				ignoreDrag = true; // ignore drag gesture with other than left
									// button 2008/04/15
			}
		}
	}
	...
	private boolean processMouseReleased(MouseEvent e) {
		...
		// Component underMouse = findComponentAt(desktop, dragPoint.x,
		// dragPoint.y);
		...
		// getShapePainter(underMouse, dragSource);
		...
		if(targetDesktop == null) {
			...
			if(underMouseNullDesktop != null) { // we've found a dock drop
												// receiver
		...
		if(deskBounds.contains(dragPoint) && ! innerBounds.contains(dragPoint)) {
			// at less than 5 pixels from a order, promote DockingPanel as the
			// receiver
			underMouse = dp;
		}
		...
		if(underMouse == null) {
			// || isAncestorOf(dockableDragSource.getDockableContainer(),
			// underMouse)){ 2007/01/08 moved to implementations
			// it's not possible to drop a parent on one of its children
			if(underMouse instanceof DockDropReceiver) {
				// but we still have to use the reference on drop receiver to
				// disaplay a floating drag shape
		...
		} else if(underMouse instanceof DockDropReceiver && e.isControlDown()) { // 2005/11/08
																					// HOT
																					// SWAP
																					// FUNCTION
			...
			if(event.isDragAccepted() && process.isDockingActionAccepted()) {// triggers
																				// vetoable
																				// listeners
				return process.dropIfPossible();
			} else if(process.canDockableBeDetached() // Not accepted on the
														// desktop //2005/10/07
	...
	protected void processHotSwap(Component underMouse, Component dragged, ShapePainterStrategy shapePainterStrategy, boolean drop) { // 2005/11/08
		...
		// ---------------------
		// shortcut : if the underMouse component doesn't belong to a
		// DockingPanel, we
		// just cancel the hot swap operation
		// explanation : hotswap between DOCKED and FLOATING whould otherwise
		// mean
		// swapping dockable states, and this will not be allowed until a
		// further release
		...
		// to implement the "hot swap" we need to find a top level dockable
		// container
		...
		if(underMouse != null) {
			...
			if(splitChild != null && splitChild != underMouse && underMouse instanceof DockDropReceiver) {// this
																											// one
																											// should
																											// always
																											// be
	...
	private void processDragGestureStart(MouseEvent e) {
		...
		if(dragSource.startDragComponent(startDragPoint)) {
			...
			this.startDragPoint = new Point(dragPoint); // same coordinate
														// system for future use
			processMouseDragged(e); // forward the event to the actual
									// processing
		} else { // drag rejected
			this.ignoreDrag = true;
		}
	...
	protected UnderMouseInfo findComponentUnderMouse(MouseEvent e) {
		...
		for(int i = 0; i < children.length; i++) {
			if(children[i] instanceof FloatingDockableContainer && children[i].isVisible()) {
				Window wChild = (Window) children[i]; // assumed by the
														// FloatingDockableContainer
														// interface
				Rectangle bounds = wChild.getBounds();
				if(bounds.contains(p)) {
					// an owned window is on top of the desktop, at current
					// mouse position
					...
					umInfo.underMouse = findComponentAt(contentPane, p2.x, p2.y); // bypass
																					// the
																					// glasspane
		...
		if(umInfo.underMouse == null && desktop.getContext().getDesktopList().size() > 1) {
			...
			// now we have an unordered list of windows all intersecting our
			// point : find which one
			// is above
			...
			if(topWindow != null) {
				...
				umInfo.underMouse = findComponentAt(contentPane, p2.x, p2.y); // bypass
																				// the
																				// glasspane
				// now find the desktop associated to the component (there might
				// be more than 1 desktop / window)
			...
			// @todo : also check for floating windows belonging to these other
			// desktops
		}
	...
	protected Component findDropReceiverOutsideDesktop(Component underMouse, MouseEvent e) {
		// there's a side case we have to manage : when a DockDropReceiver has
		// been
		// installed *outside* a desktop (this is desired when you want to
		// listen to drag and drop events (e.g. for a workspace switcher)
		// note : the event will be sent to the DockDropReceiver, but just for
		// Drag
		// (ignored as a drop)
	...
	protected void processMouseDragged(MouseEvent e) {
		...
		if(dragPoint == null || startDragPoint == null) {
			// 2006/03/16
			// safety net again : a bug has been submitted reagrding a NPE and I
			// can't reproduce it
			ignoreDrag = true;
			return;
		}
		...
		if(deskBounds.contains(dragPoint) && ! innerBounds.contains(dragPoint) && underMouse != null && targetDesktop.isAncestorOf(underMouse)) {
			// at less than 5 pixels from a border, promote DockingPanel as the
			// receiver
			underMouse = dp;
		}
		...
		if(underMouse == null) {// ||
			...
			// a state change occur when switching between CLOSED, HIDDEN, and
			// DOCKED
			// in this context, the drag event can only be generated by a hidden
			// dockable
			// (if there is a way to drag its button) or an already docked
			// dockable (no state change)
	...
	class DragProcess {
		...
		private boolean isSingleDockingActionAccepted(Dockable dockable) {
			...
			// (if there is a way to drag its button) or an already docked
			// dockable (no state change)
			if(currentState.getLocation() != futureLocation) { // state cannot
																// be null
			...
			if(dockingActionEvent instanceof DockingActionDockableEvent) {
				...
				if(dde.getDockable() == dockable) {
					...
					// multiple dockable moved at the same time (tabs) : we
					// create copies
					// of the original DockingActionEvent
					// @todo : see if we couldn't create a new type of action
					// instead
				...
				// we're dragging a full tab dockable around : accept it always
				// @todo check this
				return true;
			} else {
		...
		public boolean canDockableBeDetached() {
			if(source.getDockableContainer() instanceof TabbedDockableContainer) {
				...
				// last check : the tab container must not be itself already
				// detached
		...
		private boolean canSingleDockableBeDetached(Dockable dockable) {
			...
			if(key.isFloatEnabled()) {
				if(key.getLocation() != DockableState.Location.MAXIMIZED) {
					 ...
					 * true; }
					 */
					return true; // the test above has been moved up to filter
									// more drag events
				} else {
					return false;
				}
		...
		public boolean checkAndDetachDockable(ShapePainterStrategy shapePainterStrategy) {
			if(checkDockableWillBeDetached()) {
				if(DockingPreferences.isLightWeightUsageEnabled() && paintFloatingDragShape) {
					...
					if(dropReceiver != null) { // we are above a drop receiver
												// and we can show something
	...
	class DropProcess extends DragProcess {
		...
		private void fireSingleDockingActionEvent(Dockable dockable) {
			...
			if(dockingActionEvent instanceof DockingActionDockableEvent) {
				...
				// we're dropping a whole container (currently it is only
				// possible with

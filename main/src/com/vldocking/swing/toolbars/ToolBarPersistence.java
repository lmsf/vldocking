...
/**
 * This class contains methods to keep a set of toolbars persistent. <p> The
 * class uses the preferences framework of the java runtime, and installs a
 * shutdown hook to save the toolbars state on application exit. <p> Warning :
 * this class cannot be run from a sandboxed (untrusted) environment (unsigned
 * applet/ java web start) as it uses classes requiring a SecurityManager
 * (Preferences API and Runtime shutdown hook). <p> Call loadToolBarLayout()
 * upon application startup, after registering all ToolBars.
 * 
...
public class ToolBarPersistence implements Runnable {
	 ...
	 *            generate a reasonable name for the preferences to store
	 *            toolbar configuration
	 * @param container the toolbarcontainer to keep persistent
	 * @throws SecurityException if this class is invoked from an untrusted
	 *             environment
	 */
	...
	/**
	 * Save the toolbar configuration. <p> Masks all errors. <p> don't call this
	 * method directly : it is used by the shudown hook.
	 * 

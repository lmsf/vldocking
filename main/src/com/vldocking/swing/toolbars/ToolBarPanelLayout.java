...
public class ToolBarPanelLayout implements LayoutManager2 {
	...
	private ArrayList<ComponentInfo> componentInfos = new ArrayList<ComponentInfo>();
	...
	/**
	 * Returns the alignment of this layout (<code>FlowLayout.LEADING,
	 * TRAILING</code>...)
	 */
	public int getAlign() {
		return align;
	...
	public ToolBarConstraints getConstraints(Component comp) {
		for(int i = 0; i < componentInfos.size(); i++) {
			ComponentInfo ci = componentInfos.get(i);
			if(ci.comp == comp) {
	...
	public Dimension minimumLayoutSize(Container parent) {
		...
		synchronized(parent.getTreeLock()) {
			...
			for(int i = 0; i < majorCount; i++) {
				...
				ArrayList<ComponentInfo> minorList = getMinorComponents(i);
				for(int j = 0; j < minorList.size(); j++) {
					ComponentInfo ci = minorList.get(j);
		...
		synchronized(target.getTreeLock()) {
			...
			if(isHorizontal) {
				...
				for(int i = 0; i < majorCount; i++) {
					...
					/*
					 * for (int j=0; j < minorList.size(); j++){ ComponentInfo
					 * ci = (ComponentInfo) minorList.get(j); Component comp =
					 * ci.comp; if (comp.isVisible()){ Dimension d =
					 * comp.getPreferredSize(); comp.setBounds(left, top,
					 * d.width, d.height); left += d.width + gap; maxHeight =
					 * Math.max(maxHeight, d.height); } }
					 */
					if(ltr) {
						for(int j = 0; j < minorList.size(); j++) {
							ComponentInfo ci = minorList.get(j);
				...
				int majorCount = getMajorCount();
				/*
				 * int left = insets.left; for (int i=0; i < majorCount; i++){
				 * int top = insets.top; int maxWidth = 0; ArrayList minorList =
				 * getMinorComponents(i); for (int j=0; j < minorList.size();
				 * j++){ ComponentInfo ci = (ComponentInfo) minorList.get(j);
				 * Component comp = ci.comp; if (comp.isVisible()){ Dimension d
				 * = comp.getPreferredSize(); comp.setBounds(left, top, d.width,
				 * d.height); top += d.height + gap; maxWidth =
	...
	private void reorderComponents() {
		 ...
		 * System.out.println
		 * ("---------------------------------------------------");
		 * System.out.println("List of components : ----BEFORE ------"); for
		 * (int i=0; i < componentInfos.size(); i++){ ComponentInfo ci =
		 * (ComponentInfo)componentInfos.get(i);
		...
		for(int i = 0; i < componentInfos.size(); i++) {
			...
			if(ci.constraints.majorOffset < 0) {
				...
				for(int j = i + 1; j < componentInfos.size(); j++) {
					ComponentInfo next = componentInfos.get(j);
					next.constraints.majorOrder++;
				}
			} else if(ci.constraints.majorOffset > 0) {
				ci.constraints.majorOffset = 0;
				for(int j = i; j < componentInfos.size(); j++) { // also
																	// increase
																	// the
																	// current
																	// one
		...
		/*
		 * System.out.println("List of components : after offsets"); for (int
		 * i=0; i < componentInfos.size(); i++){ ComponentInfo ci =
		 * (ComponentInfo)componentInfos.get(i);
		 * System.out.println(ci.constraints + " = " + ci.comp); }
		 */
		...
		/*
		 * System.out.println("List of components : ------AFTER ----"); for (int
		 * i=0; i < componentInfos.size(); i++){ ComponentInfo ci =
		 * (ComponentInfo)componentInfos.get(i);
		 * System.out.println(ci.constraints + " = " + ci.comp); }
		 */
	...
	private ToolBarConstraints getHorizontalInsertionContraintsAt(Component draggedComponent, Point p) {
		...
		for(int i = 0; i < majorCount; i++) {
			...
			if(p.y < bounds.y) { // insert before first
				...
				tc.majorOffset = - 1;
				return tc;
			} else if(p.y > bounds.y + bounds.height) { // insert after.. ignore
														// (will be tested on
														// next loop)

			} else if(p.y > bounds.y + bounds.height - 5) { // insert a new
															// major as we are
															// just below the
															// lower part
				tc.majorOrder = i;
					...
					if(comp.isVisible()) {
						if(p.x < comp.getX() + comp.getWidth()) {
							if(comp == draggedComponent) {
								return null;
							} else if(p.x < comp.getX() + 10) { // at the
																// beginning of
																// the component
								tc.majorOrder = i;
								tc.minorOrder = j;
								tc.minorOffset = - 1;
								return tc;
							} else { // on the component but not at the
										// beginning
								if(j < minorList.size() - 1) {
									ComponentInfo next = minorList.get(j + 1);
	...
	private ToolBarConstraints getVerticalInsertionContraintsAt(Component draggedComponent, Point p) {
		...
		for(int i = 0; i < majorCount; i++) {
			...
			if(p.x < bounds.x) { // insert before first
				...
				tc.majorOffset = - 1;
				return tc;
			} else if(p.x > bounds.x + bounds.width) { // insert after.. ignore
														// (will be tested on
														// next loop)

			} else if(p.x > bounds.x + bounds.width - 3) { // insert a new major
															// as we are just
															// below the lower
															// part
				tc.majorOrder = i;
				...
				for(int j = 0; j < minorList.size(); j++) {
					...
					if(comp.isVisible()) {
						if(p.y < comp.getY() + comp.getHeight()) {
							if(comp == draggedComponent) {
								return null;
							} else if(p.y < comp.getY() + 10) { // at the
																// beginning of
																// the component
								tc.majorOrder = i;
								tc.minorOrder = j;
	...
	private ArrayList<ComponentInfo> getMinorComponents(int majorOrder) {
	...
	private class ComponentInfo implements Comparable<Object> {

...
/**
 * An abstract event class describing a docking action. <p> This event is used
 * to explain a dockable action : it doesn't contain any information about the
 * current state of a dockable, but describes precisely what we want to do with
 * it (for example, detach it, or left-split another dockable). <p> The
 * DockingActionDockableEvent abstract subclass is used as a superclass for all
 * events associated to a single dockable. <p> Concrete sub classes are used for
 * every type of docking action (split, tab...)
 * 
 * @since 2.1
 * 
...
public abstract class DockingActionEvent implements Cloneable {
	 ...
	 * @param initialState the initial (before action) DockableState
	 *            (DockableState.STATE_FLOATING...)
	 * @param nextState the next (after action) DockableState
	 *            (DockableState.STATE_DOCKED...)
	 * @param actionType a field defining the type of action used
	 *            (ACTION_CLOSE...)
	 */
	...
	/**
	 * Returns one of the DockableState states reflecting the state of the
	 * dockable before the action
	 */
	 ...
	 * <p> The desktop used as a source (on multi-desktop applications) can be
	 * retrieved by asking for the current dockable state (to the
	 * DockingContext).
	 */

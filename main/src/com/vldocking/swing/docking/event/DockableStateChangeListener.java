...
/**
 * Interface implemented by classes that need to be notified of a state change
 * of a dockable. <p> Events are triggered when a Dockable component is : <ul>
 * <LI> Docked (added to the desktop) <LI> set in Auto-hide mode (collapsed as a
 * border button) <LI> closed (removed from the desktop) <LI> maximized (the
 * only one visible) <LI> floating (detached from the desktop) </UL> <p>
 * 
 * 
...
public interface DockableStateChangeListener {
	...
	/**
	 * this method will be called after a dockable component has changed of
	 * state (closed, hidden, docked)
	 */

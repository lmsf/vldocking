...
/**
 * A DockableContainer is the base interface of GUI components used to display
 * <code>Dockable</code> components. <p> The components are usually displayed
 * with decorations (a title bar, docking state management buttons, a drop
 * shadow, ...) <p> This interface is meant for API extenders that would like to
 * create new kind of containers (via the sub-interfaces
 * {@link SingleDockableContainer} and {@link TabbedDockableContainer} and the
 * {@link DockableContainerFactory}). <p> Usually, a DockableContainer also
 * contains (or implements) one ore more <code>DockableDragSource</code>s in
 * order to achieve drag and drop operations. <p>
 * 
...
public interface DockableContainer {
	...
	/**
	 * Invoked once after creation, in order to let this component register its
	 * DockableDragSources to the DockingDesktop, and have a reference of this
	 * Desktop.
	 * 

...
public class BorderSplitter {
	...
	public void split(DockDropEvent event, Container draggedContainer, DockingConstants.Split position) {
		/* standard splitting */
		// event.getDesktop().splitComponent(delegator,
	...
	private void acceptSplit(DockDropEvent event, DockingConstants.Split position) {
		...
		if(container instanceof TabbedDockableContainer) {
			...
			// if (tabAncestor != null && tabAncestor.getTabCount() == 1 &&
			...
			if(tabAncestor != null && tabAncestor.getTabCount() == 0 && tabAncestor == delegator) {// 2007//11/15
				...
				// the drop gesture comes from the tabAncestor (as it's given by
				// the drag source)
				// and translates into a border split
				// if only one tab remains, that means the tabAncestor is no
				// longer useful and
				// so we now need to remove that tab and replace it by a
				// standard singledockable
				// which is done by the standard desktop "split" public method.
				// Dockable remainingDockable = tabAncestor.getDockableAt(0);
				event.getDesktop().split(remainingDockable, // the only
															// remaining
															// dockable
				dockable, position);
			} else {
				// standard dropping, just use the internal API to relayout the
				// desktop.

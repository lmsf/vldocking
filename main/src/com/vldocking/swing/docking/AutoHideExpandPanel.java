...
public class AutoHideExpandPanel extends JPanel implements SingleDockableContainer {
	...
	// true during animation phase, will be used to avoid concurrent animations
	// (@todo)
	...
	private JPanel topDragger = getTopDragger(); // indirection because those
	...
	private JPanel content = new JPanel(new BorderLayout()); // content
																// displayed
																// (user
																// component)

	private Panel heavyPanel; // used only when mixing lightweight and
	...
	= new javax.swing.Timer(250, new ActionListener() {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if(expandPanelBounds.contains(mouseLocation)) {
				...
				// not inside the component : check also into its associated
				// button
	...
	public AutoHideExpandPanel() {
		...
		// requiered to trap mouse events under the panel (so they don't go to
		// the mousegrabber)
		...
		if(DockingPreferences.isLightWeightUsageEnabled()) {
			...
			if(! DockingPreferences.isSingleHeavyWeightComponent()) {
				...
				// to ensure our content will not be covered by an underlying
				// heavy component.
				// to do this, we add an intermediary heavyweight panel.
				// note : if there is only one heavyweight dockable,
				// we delay and delegate this operation to
				// installHeavyWeightIfNeeded
				...
				// (heavy + single) will be dynamically changed depending on the
				// expanded dockable
				add(content, BorderLayout.CENTER);
		...
		addAncestorListener(new AncestorListener() { // 2006/12/19 : reworked to
														// avoid GC leak
	...
	public boolean isActive() {
		if(titleBar.isActive()) {
			...
			// since 2.1 : the autohide component can contain a nested set of
			// dockables, so the
			// titlebar's activity isn't always usable : we have to verify if
			// the focus is still
			// inside
	...
	public void expand() {
		if(selectedButton == null) {
			return;// 2007/01/10
			// throw new
			// IllegalStateException("No button selected for expansion");
		}
	...
	private void installComponent() {
		...
		if(expandedComponent != null) {
			if(expandedComponent != comp) {
				content.remove(expandedComponent);
				content.add(comp, BorderLayout.CENTER); // 2005/12/08 back again
														// in this order (JDIC
														// workaround)
		...
		public void mouseClicked(MouseEvent e) {
			...
			if(! isExpanding) {
				...
				if(selectedButton != clicked) {
					// we need to tell the focus listener not to manage focus
					// while
					// this operation is occuring
					isUnderControl = true;
					select(clicked); // will trigger a focus change is
										// DockingPreferences.isHeavuWeight
					SwingUtilities.invokeLater(new Runnable() {
		...
		public void propertyChange(PropertyChangeEvent e) {
			if(! controler.isUnderControl) {
				...
				if(c != null && ! AutoHideExpandPanel.this.isAncestorOf(c)) {
					...
					// avoid collapsing when focus is given to a component which
					// isn't a dockable

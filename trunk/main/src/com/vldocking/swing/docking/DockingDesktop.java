...
public class DockingDesktop extends JLayeredPane {
	...
	private HashMap<DockKey, AutoHideButton> autoHideButtons = new HashMap<DockKey, AutoHideButton>(); // key
																										// :
																										// DockKey
																										// /
																										// value
	...
	protected HashMap<Dockable, DockableState> previousFloatingDockableStates = new HashMap<Dockable, DockableState>(); // key
																														// dockable
																														// /
																														// value
																														// dockable
	...
	public DockingDesktop(String desktopName, DockingContext context) {
		...
		addAncestorListener(new AncestorListener() {
			...
			public void ancestorAdded(AncestorEvent event) {
				KeyboardFocusManager.getCurrentKeyboardFocusManager().addPropertyChangeListener("focusOwner", focusHandler); // 2006/08/21
																																// :
																																// back
																																// to
																																// life
			...
			public void ancestorRemoved(AncestorEvent event) {// 2005/12/08
				...
				if(event == null || event.getAncestorParent() == null) {
					...
					if(w != null) { // 2007/03/07 hope it helps with desktops on
									// JDialogs
		...
		if(floating) {
			...
			switch(location) {
				case DOCKED:
					...
					break;
				case HIDDEN:
					position = currentState.getPosition(); // get the position
															// that was stored
															// before
					break;
				case FLOATING:
	...
	private void xmlWriteCompoundDockable(CompoundDockable dockable, PrintWriter out) throws IOException {
		...
		DockKey key = dockable.getDockKey();
		out.println("<Key dockName=\"" + key.getKey() + "\"/>");
		CompoundDockingPanel cdp = (CompoundDockingPanel) dockable.getComponent(); // no
																					// choice
																					// here
		if(cdp.getComponentCount() > 0) {
			// compound panels cannont have more than one child

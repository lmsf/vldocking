...
/**
 * An event triggered after a change of dockable state. <P> Events are triggered
 * when a component is : <UL> <LI> Docked (added to the desktop) <LI> set in
 * Auto-hide mode (collapsed as a border button) <LI> closed (removed from the
 * desktop) <LI> maximized (the only one visible) <LI> floating (detached from
 * the desktop) </UL>
 * 
 * <P> It is also possible to track changes before they occurs with the
 * DockableStateWillChangeEvent, which is triggered before the change is

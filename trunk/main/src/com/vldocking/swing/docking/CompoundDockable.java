...
/**
 * Describes a Dockable that can contain sub-dockables. <p> This class allows
 * dockable nesting, with full support for workspace save and restore. <p>
 * Currently this class is associated to a CompoundDockingPanel component : you
 * insert dockables into it with an initial addDockable(CompoundDockable)
 * invocation from DockingDesktop, and further dockables with split/createTab
 * calls. <p> Example : <pre> DockingDesktop desk = ...
 * desk.addDockable(dockable1); desk.split(dockable1, dockable2,
 * DockingConstants.SPLIT_RIGHT); // now let's add a compound dockable
 * CompoundDockable compound = new CompoundDockable(new DockKey("Compound!"));
 * desk.split(dockable1, compound); // and add a tab into it
 * desk.addDockable(compound, dockable3); // initial nesting : new API call
 * desk.createTab(dockable3, dockable4, 1); // a tab, using standard API </pre>
 * <p> CompoundDockables are displayed as SingleDockableContainers (with a title
 * bar). Don't forget to properly initialize the DockKey of this dockable to
 * have it well presented on screen (name, tooltip, icon).
 * 
...
public class CompoundDockable implements Dockable {
	 ...
	 * 
	 * <p> Implementation note : this method always returns a

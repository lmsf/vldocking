...
/**
 * An Icon allowing the use of closeable tabs by simulating the entire tabpane
 * tab-painting.
 * 
 * <p> As JTabbedPanes cannot use any JComponents as tab selectors (the access
 * if protected and we just have a label, an icon and a tooltip), we have to
 * rely on tricks to bypass them.
 * 
 * 
 * 
 * @author Lilian Chamontin, VLSolutions
 * @update 2005/11/01 Lilian Chamontin : fixed NPE when otherIcons == null (tabs
 *         without smart icons)
 * @update 2005/11/08 Lilian Chamontin : fixed bug related to event management
 *         when multiple other-icons are used (big thanks to Emmanuel GAUVRIT).
 * @update 2005/11/21 Lilian Chamontin : enhanced width calculation of the icon
 *         size.
 */
public class JTabbedPaneSmartIcon implements Icon, Cloneable {

	/*
	 * Implementation : This icon is larger than standard icons : it also paints
	 * the tab title and optional buttons. Inner Event management (with
	 * JTabbedPaneSmartIconManager) allows simulation of action on the buttons.
	 */
	...
	/**
	 * local tooltip text : the one to use during mouse movements (depends on
	 * the mouse position, and can be either the tab selector tooltip text or
	 * one of the smart buttons included into this icon).
	 */
	...
	/*
	 * The container this icon is for (required to calculate proper widths and
	 * heights)
	 */
	...
	public JTabbedPaneSmartIcon(Icon icon, String label, SmartIconJButton[] otherIcons) {
		...
		} catch(SecurityException ignore) { // for untrusted web start apps
											// failing gracefully
		}
	...
	/**
	 * Return the local tooltip of this icon (the one associated with inner
	 * mouse movements)
	 */
	...
	/**
	 * Specify which container will use this icon. <p> If the icon is shared
	 * between containers, please provide at least one as this allows the icon
	 * to properly estimate its dimension.
	 */
	...
	public void paintIcon(Component c, Graphics g, int x, int y) {
		...
		// reevaluate the width with correct graphics object, in case something
		// has changed
	...
	private SmartIconJButton findButtonAt(Point p) {
		...
		if(otherIcons != null) { // 2005/11/01
			for(int i = 0; i < otherIcons.length; i++) {
				...
				start += btn.getIconWidth(); // bug corrected thanks to Emmanuel
												// GAUVRIT 2005/11/08
				if(i < otherIcons.length - 1) {
	...
	public int getIconWidth() {
		if(width == - 1) {
			...
			// reevaluate the width with correct graphics object, in case
			...
			if(otherIcons != null) {
				for(int i = 0; i < otherIcons.length; i++) {
					width += otherIcons[i].getIconWidth(); // additional width
															// for icons
					if(i < otherIcons.length - 1) {
	...
	public int getIconHeight() {
		if(height == - 1) {
			if(icon != null) {
				...
				height = 16; // standard height (as if there was an icon) :
								// should be calculated instead of fixed
			}
		}
		return height;
	}
	...
	/**
	 * Process the mouse pressed event. <p> Mouse coordinates are given relative
	 * to this icon
	 */
	...
	/**
	 * Process the mouse released event. <p> Mouse coordinates are given
	 * relative to this icon
	 */
	...
	/**
	 * Process the mouse exited event. <p> Mouse coordinates are given relative
	 * to this icon
	 */
	...
	/**
	 * Process the mouse moved event. <p> Mouse coordinates are given relative
	 * to this icon
	 */

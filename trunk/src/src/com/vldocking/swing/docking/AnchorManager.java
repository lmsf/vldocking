...
public class AnchorManager {
	...
	private boolean acceptSplitComponent(DockingActionSplitComponentEvent event) {
		...
		// we suppose that dockables in "base" are ok (respecting their own set
		// of constraints before the split)
		...
		// we have to check if the moved dockable will respect its own
		// constraints
	...
	private boolean acceptSplitDockable(DockingActionSplitDockableEvent event) {
		...
		// the base dockable, once the new component added, must also respect
	...
	private boolean acceptSplitDockableContainer(DockingActionSplitDockableContainerEvent event) {
		...
		// now, we're sure we won't break a 'base' constraint
		// we have to check if the moved dockable container will respect its own
		// constraints

		if(event.getDockableContainer() instanceof TabbedDockableContainer) {
			// the only one managed currently (drag gesture : move a whole
			// tabbed pane around)
			...
			for(int i = 0; i < containersDockables.size(); i++) {
				...
				if(acDockable != null) {
					int anchor = acDockable.getAnchor();
					if((anchor & contactBorders) != anchor) { // anchor is not
																// respected
						return false;
					}
				}

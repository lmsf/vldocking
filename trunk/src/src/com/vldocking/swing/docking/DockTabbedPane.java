...
public class DockTabbedPane extends JTabbedPane implements DockDropReceiver, DockableDragSource, TabbedDockableContainer {
	...
	private HashMap<DockKey, Action> closeActions = new HashMap<DockKey, Action>(); // DockKey
																					// ,
																					// Action

	// private JTabbedPaneSmartIconManager tabManager = new
	// JTabbedPaneSmartIconManager(this);
			...
			if(e.getPropertyName().equals(DockKey.PROPERTY_CLOSEABLE)) {
				...
				if(tab >= 0) {
					...
					SwingUtilities.invokeLater(new Runnable() {
						...
						public void run() {
							// we have to revalidate/repaint to ensure the new
							// icon size is correctly
							// taken into account
							revalidate();
							repaint();
						}
				...
				if(tab >= 0) {
					...
					setToolTipTextAt(tab, (String) e.getNewValue()); // 2008/04/15
																		// also
																		// propagate
																		// to
																		// the
	...
	public DockTabbedPane() {
		...
		addMouseListener(new MouseAdapter() {
			...
			// depending on the platform, pop-up is triggered either by
			...
			public void mouseClicked(MouseEvent e) {
				if(e.getClickCount() == 2 && SwingUtilities.isLeftMouseButton(e)) {
					...
					if(d != null && d.getDockKey().isMaximizeEnabled() && d.getDockKey().getLocation() == DockableState.Location.DOCKED) { // 2005/10/10
																																			// don't
																																			// maximize
																																			// floatings
						desktop.maximize(d);
		...
		if(d != null) {
			...
			if(key.getLocation() == DockableState.Location.DOCKED) {
				...
				if(key.isCloseEnabled()) {
					...
					// mItem.setPressedIcon(new
					// ImageIcon(getClass().getResource("close16.gif")));
					popup.add(mItem);
					menuCount++;
				}
			} else if(key.getLocation() == DockableState.Location.FLOATING) {
				if(DockingUtilities.isChildOfCompoundDockable(d)) {
					// as of v2.1 floating dockable which is a child of compound
					// dockable don't have "attach" menu
				} else {
				...
				// a tabbed hidden dockable can only occur when the dockable is
	...
	private void addDockableContainer(final SingleDockableContainer dc, int tab) {
		...
		if(tab >= getTabCount()) {
			// addTab(key.getName(), key.getIcon(), (Component)dc,
			// key.getTooltip());
			addTab("", smartIcon, (Component) dc, key.getTooltip());
			// addTab(key.getName(),smartIcon, (Component) dc,
			// key.getTooltip());
			tab = getTabCount() - 1;
		} else {
			// insertTab(key.getName(), key.getIcon(), (Component)dc,
			// key.getTooltip(), tab);
			insertTab("", smartIcon, (Component) dc, key.getTooltip(), tab);
			// insertTab(key.getName(), smartIcon, (Component)dc,
			// key.getTooltip(), tab);
		}
	...
	private void scanDrop(DockEvent event, boolean drop) {
		...
		if(dragSource.getDockableContainer().isAncestorOf(this)) {
			// this is possible for compound containers (as they contain
			// sub-dockables)
			// in that case, you cannnot drop a compound into one of its
			// children // 2007/01/08
			if(drop) {
			...
			if(tabbounds.contains(p)) {
				...
				if(drop) {
					if(DockingUtilities.findTabbedDockableContainer(dockable) == this) {
						// the dockable IS another tab of this container, so we
						// just have to
						// change its tab order.
						((DockDropEvent) event).acceptDrop(false); // don't
																	// remove
						...
						if(base.getDockKey().getLocation() == DockableState.Location.FLOATING) { // 2005/12/09
							if(dragSource.getDockable().getDockKey().getLocation() == DockableState.Location.FLOATING) {
								// this is new 2.1 feature : allowed DnD if the
								// tab is a child of a compund dockable
						...
						// addDockable(dragSource.getDockable(), i);
						// //2005/10/08
					}
					setSelectedIndex(i);
				} else {
		...
		if(afterlast.contains(p)) {
			...
			if(drop) {
				...
				if(DockingUtilities.findTabbedDockableContainer(dockable) == this) {
					// the dockable IS another tab of this container, so we just
					// have to
					// change its tab order.
					...
					if(base.getDockKey().getLocation() == DockableState.Location.FLOATING) { // 2005/12/09
						if(dragSource.getDockable().getDockKey().getLocation() == DockableState.Location.FLOATING) {
							// this is new 2.1 feature : allowed DnD if the tab
							// is a child of a compund dockable
							((DockDropEvent) event).acceptDrop(); // remove
						} else {
							((DockDropEvent) event).acceptDrop(false); // don't
																		// remove
		...
		if(getTabPlacement() == SwingConstants.BOTTOM) {
			...
			if(getTabRunCount() > 1) {
				...
				if(afterlast.x + 30 > vbounds.x + vbounds.width) {
					// we're outside the component bounds, so we go back a
					// little bit
	...
	private void scanMainTabZone(DockEvent event, boolean drop, DockableDragSource dragSource, Rectangle vbounds) {
		...
		if(drop) {
			...
			if(base.getDockKey().getLocation() == DockableState.Location.FLOATING) {// 2005/12/09
				if(dragSource.getDockable().getDockKey().getLocation() == DockableState.Location.FLOATING) {
					// this is new 2.1 feature : allowed DnD if the tab is a
					// child of a compound dockable
					((DockDropEvent) event).acceptDrop(); // remove
				} else {
					((DockDropEvent) event).acceptDrop(false); // don't remove
			...
			// event.getDesktop().addToTabbedGroup(base,
			// event.getDragSource().getDockable()); //2005/07/13
	...
	private boolean scanBorderBounds(DockEvent event, boolean drop, Point p) {
		...
		if(firstDockable.getDockKey().getLocation() == DockableState.Location.FLOATING) {
			// as of v2.1 this DnD operation is allowed if the tabbed pane is a
			// child of a compund dockable
		...
		if(getTabPlacement() == SwingConstants.BOTTOM) {
			...
			int yTab = tabBounds.y + tabBounds.height; // lower coordinate of
														// tabbed selectors
	...
	public boolean startDragComponent(Point p) {
		...
		for(int i = 0; i < getTabCount(); i++) {
			...
			if(tabbounds.contains(p)) {
				...
				if(i > 2 && i == getTabCount() - 1) {
					// workaround for a focus problem : when the JTabbedPane has
					// focus on the last tab
					// and we start a drag (and drop outside the tabbedpane,
					// there is a
					// nonblocking stacktrace due to a bad focusIndex in
					// BasicTabbedPaneUI (method focusLost() of the Handler)
					KeyboardFocusManager.getCurrentKeyboardFocusManager().upFocusCycle();
					// by putting the focus up before the drop occurs, we ensure
					// a good focusIndex is set
				}
				return true;
			}
		}
		...
		Rectangle afterlast = new Rectangle(lasttabbounds.x + lasttabbounds.width, lasttabbounds.y, getX() + getWidth() - (lasttabbounds.x + lasttabbounds.width), lasttabbounds.height);
		if(afterlast.contains(p) || header.contains(p)) { // either after the
															// last tab or
															// between tabs
	...
	public Dockable getSelectedDockable() {
		...
		if(comp instanceof SingleDockableContainer) {
			...
			// this may happen when the dockable is maximized : it is
	...
	public void removeDockable(Dockable dockable) {
		...
		if(dc != null) {
			// after a dockable is removed, we go back to the previously
			// selected if still visible

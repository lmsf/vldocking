...
class HeavyWeightDragControler extends AbstractDragControler {
	...
	protected Component findComponentAt(Container container, int x, int y) {
		...
		// getShapePainter(container, null);
			...
			if(child.isVisible()) {
				...
				if(bounds.contains(x, y)) {
					if(child instanceof Container) {
						...
						// // skip our dedicated components for heavyweight
						// support
						// return child;
					}
				}
	...
	// -----------------------------------------------------------------------------------------------------------
	// --------------------------------- you don't want to look at the code
	// underneath this line... --------------
	// -----------------------------------------------------------------------------------------------------------
	// I'll rewrite it entirely someday as i'm not pleased with it (much too
	// complex)
	/**
	...
	private class HWShapePainterStrategy implements ShapePainterStrategy {
		...
		// heavyweight support
		private HeavyShape heavyShape; // instanciated only when heavyweight
										// suport is enabled
		...
		public void repaint() {
			 ...
			 */
			if(dropShape != null) {
				heavyShape.moveToShape(dropShape); // adjust to the shape before
													// repainting
		...
		public void startDrag(DockableDragSource source) {
			...
			// take a snapshot of the frame... ugly trick, but couldn't find
			// anything better !
	...
	private class HeavyShape extends Canvas {
		...
		public void moveToShape(Shape newShape) {
			...
			r.x += p.x;
			r.y += p.y;
			r.width += 2; // shape has stroke(3), so we must extend it a little
							// bit
			r.height += 2;
			if(r.x + r.width > container.getWidth()) { // check extend not out
														// of bounds
	...
	private class HeavyLabel extends Canvas {
		...
		public void setZOrder() {
			...
			try {
				// desktop.setComponentZOrder(this, -2); // on top of heavy
				// panel
				Method m = Container.class.getMethod("setComponentZOrder", new Class[] {Component.class, int.class});
				m.invoke(getParent(), new Object[] {this, new Integer(0)});
			} catch(Exception ignore) {

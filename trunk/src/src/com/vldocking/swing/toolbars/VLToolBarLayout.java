...
/**
 * The layout for VLToolBar (supports horizontal and vertical orientation). <p>
 * When used in horizontal orientation, this layout respects the preferred width
 * of the contained components, and forces the height to the maximum of all the
 * preferred heights. (so components are vertically streched to have the same
 * height). <p> When used in vertical orientation, this layout respects the
 * preferred height of the contained components, and forces the width to the
 * maximum of all the preferred widths. (so components are horizontally streched
	...
	public void layoutContainer(Container target) {
		synchronized(target.getTreeLock()) {
			...
			if(isHorizontal) {
				...
				/*
				 * for (int i = 0; i < components.size(); i++) { Component comp
				 * = (Component) components.get(i); if (comp.isVisible()){
				 * Dimension d = comp.getPreferredSize(); comp.setBounds(left,
				 * insets.top, d.width, toolbarDim.height - insets.top -
				 * insets.bottom); left += d.width + gap; } }
				 */
				...
				// int centerX = insets.left + (toolbarDim.width - insets.left -
				// insets.right)/2;
				int top = insets.top;
				for(int i = 0; i < components.size(); i++) {
					Component comp = (Component) components.get(i);

...
/**
 * A sub component of JTabbedPaneSmartIcon, used to describe a button included
 * in a tabbedpane. <p> As JTabbedPanes cannot use any JComponents as tab
 * selectors (the access if protected and we just have a label, an icon and a
 * tooltip), we have to rely on tricks to bypass them. <p> This trick, the
 * SmartIconJButton is an icon faking the behaviour of a button. It uses an
 * Action for reacting to clicks and manages a set of images (default, rollover,
 * pressed, disabled) to behave like a rollover button.
 * 
...
public class SmartIconJButton implements Icon, PropertyChangeListener {
	...
	/**
	 * paints the appropriate icon according to its internal state (pressed,
	 * rollover...)
	 */
	...
	/**
	 * Do not call directly as it a side effect of listening to the action
	 * changes.
	 */

...
public class DockingDesktop extends JLayeredPane {
  ...
  private HashMap autoHideButtons = new HashMap(); // key : DockKey / value : AutoHideButton
  ...
  protected HashMap tabbedGroups = new HashMap(); // <Dockable>/<LinkedList<Dockable>
  
  private FocusHandler focusHandler = new FocusHandler();
  
  /** return state for floating dockables */
  protected HashMap previousFloatingDockableStates = new HashMap(); // key dockable / value dockable state
    ...
    public void actionPerformed(ActionEvent e){
      ...
      if (d != null){
        if (d.getDockKey().isCloseEnabled()){
          ...
          if (state == DockableState.STATE_DOCKED || state == DockableState.STATE_FLOATING
              || state == DockableState.STATE_HIDDEN){
            close(d);
          }
        }
      }
      ...
      if (d != null){
        if (d.getDockKey().isMaximizeEnabled()){
          ...
          if (state == DockableState.STATE_DOCKED){
            maximize(d);
          } else if (state == DockableState.STATE_MAXIMIZED){
            restore(d);
          }
        }
      }
    ...
    public void actionPerformed(ActionEvent e){
      ...
      if (d != null){
        if (d.getDockKey().isAutoHideEnabled()){
          ...
          if (state == DockableState.STATE_DOCKED){
            setAutoHide(d, true);
          } else if (state == DockableState.STATE_HIDDEN){
            setAutoHide(d, false);
          }
        }
      }
...
//    add(contentPane, BorderLayout.CENTER);
  ...
  private void createTab(Dockable base, Dockable dockable, int order, boolean select, boolean triggerEvents){
    ...
    int currentStateAsInt = getStateAsInt(currentState);
    ...
    if (base.getDockKey().getDockableState() == DockableState.STATE_FLOATING){
      ...
      newState = new DockableState(this, dockable,
          base.getDockKey().getDockableState(), position);
    } else {
      newState = new DockableState(this, dockable,
          base.getDockKey().getDockableState());
    }
    ...
    DockingActionEvent dae = new DockingActionCreateTabEvent(this, dockable,
        currentStateAsInt, newState.getState(), base, order);
    
    //boolean stateChange = currentState == null || !currentState.isDocked();
    //boolean stateChange = currentState == null || currentState.getState() != newState.getState(); //2005/10/07
    if (triggerEvents){
    ...
    if (currentStateAsInt != DockableState.STATE_CLOSED){
...
//    if ( currentState != null && (!currentState.isFloating()) && newState.isFloating()){
    ...
    if (triggerEvents){
      dockable.getDockKey().setDockableState(base.getDockKey().getDockableState()); //2005/10/06
  ...
  public void split(Dockable base, Dockable newDockable, DockingConstants.Split position){
    ...
    DockableState newState = new DockableState(this, newDockable,
        DockableState.STATE_DOCKED);
    DockingActionEvent dae = new DockingActionSplitDockableEvent(this, newDockable,
        currentStateAsInt, newState.getState(), base, position, 0.5f);
    ...
    DockableContainer dockableContainer = DockableContainerFactory.getFactory().
        createDockableContainer(newDockable, DockableContainerFactory.PARENT_SPLIT_CONTAINER);
...
//    baseTab = null; // V2.1 TEST
    ...
    newDockable.getDockKey().setDockableState(DockableState.STATE_DOCKED);
  ...
  public void splitComponent(Component base, Dockable dockable, DockingConstants.Split position){
    ...
    //int futureState = DockableState.STATE_DOCKED;
    int futureState = DockingUtilities.getDockableStateFromHierarchy(base);
    /*if (!dockingPanel.isAncestorOf(base)){
      futureState = DockableState.STATE_FLOATING;
    }*/
    ...
    if (currentState == null){
      stateChange = true;
    } else if (currentState.getState() != futureState){
      stateChange = true;
    }
    
    DockableState newState = new DockableState(this, dockable, futureState);
//        DockableState.STATE_DOCKED);
    DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(
        currentState, newState);
    DockingActionEvent dae = new DockingActionSplitComponentEvent(this, dockable, currentStateAsInt,
        newState.getState(), base, position, 0.5f);
    ...
    DockableContainer dockableContainer = DockableContainerFactory.getFactory().
        createDockableContainer(dockable, DockableContainerFactory.PARENT_SPLIT_CONTAINER);
    dockableContainer.installDocking(this);
...
//    }
    ...
    if (stateChange){
      if (futureState == DockableState.STATE_FLOATING ){
  ...
  private void splitTab(Component base, TabbedDockableContainer tdc, DockingConstants.Split position){
    ...
    if (stateChange){
      ...
      for (int i=0; i < tdc.getTabCount(); i++){
        ...
        DockableState newState = new DockableState(this, d, DockableState.STATE_DOCKED);
...
//    DockingUtilities.updateResizeWeights(dockingPanel);
  ...
  private void show(Dockable dockable, DockingActionEvent action){
    ...
    LinkedList group = (LinkedList) tabbedGroups.get(dockable); //2005/07/13...
...
//          position.getRelativeAncestorContainer(), dockable, position);
  ...
  public void remove(Dockable dockable) {
    ...
    if ( dockState == DockableState.STATE_DOCKED
        || dockState == DockableState.STATE_FLOATING){ // ...2005/10/06
    ...
    if (dockState == DockableState.STATE_FLOATING){
    ...
    } else if (dockState == DockableState.STATE_HIDDEN){
//      DockableState state = (DockableState) dockableStates.get(dockable);
      
//      if (state.isHidden()) {
      // component is removed from the auto-hide list
      //dockableStates.put(dockable, new DockableState(dockable, DockableState.STATE_CLOSED));
      // 2005/10/10 removed this : wrong place to change a dockable state
      
      AutoHideButton btn = (AutoHideButton) autoHideButtons.get(dockable.
          getDockKey());
...
/*      } else if (state.isFloating()){
  ...
  public void maximize(Dockable dockable){
    if (dockable.getDockKey().getDockableState() != DockableState.STATE_DOCKED){
    ...
    DockableState newState = new DockableState(this, dockable, DockableState.STATE_MAXIMIZED);
    
    int currentStateAsInt = getStateAsInt(currentState);
    DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(
        currentState, newState);
    DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, dockable,
        currentStateAsInt, DockableState.STATE_MAXIMIZED);
    ...
    .createDockableContainer(dockable, DockableContainerFactory.PARENT_DESKTOP);
    ...
    dockable.getDockKey().setDockableState(DockableState.STATE_MAXIMIZED);
    fireDockingAction(dae, new DockableStateChangeEvent(currentState, newState));
  ...
  public void restore(final Dockable dockable){
    if (dockable.getDockKey().getDockableState() != DockableState.STATE_MAXIMIZED){
      // development error (could have been an assertion).
    ...
    DockableState currentState = getDockableState(dockable);
    boolean stateChange = currentState != null || currentState.isMaximized();
    DockableState newState = new DockableState(this, dockable, DockableState.STATE_DOCKED);
    int currentStateAsInt = getStateAsInt(currentState);
    
    DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(
        currentState, newState);
    DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, dockable, currentStateAsInt, DockableState.STATE_DOCKED);
    ...
    if (dummyMaximedReplacer.getParent() instanceof TabbedDockableContainer){
      ...
      sdc = DockableContainerFactory.getFactory().createDockableContainer(dockable,
          DockableContainerFactory.PARENT_SPLIT_CONTAINER);
    }
  ...
  public void setFloating(final Dockable dockable, boolean floating, Point screenPosition){
    ...
    if (floating){
      ...
      } else if (state == DockableState.STATE_CLOSED){
        return; // ignore : to detach a dockable it must be somehow visible //2006/02/20
      } else {
        throw new IllegalArgumentException("not docked " + state);
      }
      DockableState newState = new DockableState(this, dockable, DockableState.STATE_FLOATING, position);
      DockableStateWillChangeEvent event = new DockableStateWillChangeEvent(currentState, newState);
      
      if (state == DockableState.STATE_FLOATING){
        ...
        DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, dockable, currentStateAsInt,
        ...
        if (isDockingActionAccepted(dae, event)){
          ...
          context.setDockableState(dockable, newState);
          dockable.getDockKey().setDockableState(DockableState.STATE_FLOATING);
          fireDockingAction(dae, new DockableStateChangeEvent(currentState, newState));
      ...
      DockableState newState = new DockableState(this, dockable, previousState.getState(), previousState.getPosition());
      DockableStateWillChangeEvent event = new DockableStateWillChangeEvent(currentState, newState);
      ...
      if (currentState != null && currentState.isFloating()){
        if (previousState.isDocked()){
          ...
          dae = new DockingActionSimpleStateChangeEvent(this, dockable, DockableState.STATE_FLOATING, DockableState.STATE_HIDDEN);
        } else {
        ...
        if (isDockingActionAccepted(dae, event)){
          ...
          if (dockable instanceof CompoundDockable){
            updateCompoundChildrenState((CompoundDockable)dockable, newState.getState());
          }
  ...
  void setFloating(final TabbedDockableContainer tdc, Point screenPosition){
    ...
    if (!currentState.isDocked()){
      ...
      if (checkDockableStateWillChange(tdc, DockableState.STATE_FLOATING)){
        ...
        DockingUtilities.setVisible(fdc, true);
        //dialog.setVisible(true);
        fireStateChanged(tdc, DockableState.STATE_FLOATING, position);
        DockingUtilities.updateResizeWeights(dockingPanel);
        revalidate();
      }
    }
  ...
  private boolean checkDockableStateWillChange(TabbedDockableContainer tdc, int futureState){
    for (int i=0; i < tdc.getTabCount(); i++){
      ...
      int currentStateAsInt = getStateAsInt(currentState);
      DockableState newState = new DockableState(this, d, futureState, null);
      DockableStateWillChangeEvent event = new DockableStateWillChangeEvent(currentState, newState);
      DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, d, currentStateAsInt, newState.getState());
      if (!isDockingActionAccepted(dae,event)){
        return false;
      }
    }
  ...
  private void fireStateChanged(TabbedDockableContainer tdc, int futureState, RelativeDockablePosition position){
    for (int i=0; i < tdc.getTabCount(); i++){
      ...
      DockableState currentState = getDockableState(d);
      int currentStateAsInt = getStateAsInt(currentState);
      DockableState newState = new DockableState(this, d, futureState, position);
      
      context.setDockableState(d, newState);
      d.getDockKey().setDockableState(DockableState.STATE_FLOATING);
      DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, d, currentStateAsInt, newState.getState());
      fireDockingAction(dae, new DockableStateChangeEvent(currentState, newState));
    }
  }
  ...
  public DockableState getPreviousDockableState(Dockable dockable){
    ...
    return (DockableState) previousFloatingDockableStates.get(dockable);
  }
  ...
  private DockableState removePreviousFloatingState(Dockable dockable){
    ...
    return (DockableState) previousFloatingDockableStates.remove(dockable);
  }
  private void storePreviousFloatingState(Dockable dockable, DockableState state){
    ...
    if (dockable instanceof CompoundDockable){
      ...
      for (int i=0; i < children.size(); i++){
        Dockable d = (Dockable) children.get(i);
        previousFloatingDockableStates.put(d, new DockableState(this, d, state.getState(), state.getPosition()));
      }
    }
  }
  ...
  private void floatingToHide(Dockable dockable, RelativeDockablePosition position){
    ...
    AutoHideButton btn = (AutoHideButton) autoHideButtons.get(k);
    if (btn == null) {
      ...
      try {
        if (parent instanceof SplitContainer) {
          ...
          if (tparent.getTabCount() == 1) { // no more use for tabs
            ...
            boolean floating = last.getDockKey().getDockableState() == DockableState.STATE_FLOATING;
            DockableContainer lastContainer = null;
            if (floating) {
              lastContainer = DockableContainerFactory.getFactory().
                  createDockableContainer(last,  DockableContainerFactory.PARENT_DETACHED_WINDOW);
            } else {
              lastContainer = DockableContainerFactory.getFactory().
                  createDockableContainer(last,  DockableContainerFactory.PARENT_SPLIT_CONTAINER);
            }
...
public void close(Dockable dockable){
  ...
  RelativeDockablePosition position = new RelativeDockablePosition(dockingPanel, dockable);
  DockableState newState = new DockableState(this, dockable, DockableState.STATE_CLOSED, position);
  DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
  DockingActionEvent dae = new DockingActionCloseEvent(this, dockable, currentStateAsInt);
  
  if (dockingPanel.isAncestorOf(dockable.getComponent())){
    if (isDockingActionAccepted(dae, dswe)){
      if (dockable instanceof CompoundDockable){
        // V2.1 : propagate closing state to children
        updateCompoundChildrenState((CompoundDockable)dockable, DockableState.STATE_CLOSED);
      }
    ...
    if (isDockingActionAccepted(dae, dswe)){
      ...
      context.setDockableState(dockable, new DockableState(this, dockable,
          DockableState.STATE_CLOSED, currentState.getPosition()));
      dockable.getDockKey().setDockableState(DockableState.STATE_CLOSED);
      fireDockingAction(dae, new DockableStateChangeEvent(currentState, newState));
    }
 ...
 */
private void updateCompoundChildrenState(CompoundDockable cDockable, int state){
  ArrayList children = DockingUtilities.findCompoundDockableChildren(cDockable);
  for (int i=0; i < children.size(); i++){
    Dockable d = (Dockable) children.get(i);
    d.getDockKey().setDockableState(state);
...
public void setAutoHide(Dockable dockable, boolean hide){
  ...
  if (hide){
    ...
    DockableState newState = new DockableState(this, dockable, DockableState.STATE_HIDDEN, position);
    DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
    DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, dockable,
        currentStateAsInt, DockableState.STATE_HIDDEN);
    
    if (currentStateAsInt != DockableState.STATE_HIDDEN
      ...
      hide(dockable);
      context.setDockableState(dockable, newState);
      k.setDockableState(DockableState.STATE_HIDDEN);
      fireDockingAction(dae, new DockableStateChangeEvent(currentState, newState));
      if (dockable instanceof CompoundDockable){
        // V2.1 : also trigger a dockable state change event for compound dockable children
        updateCompoundChildrenState((CompoundDockable) dockable, DockableState.STATE_HIDDEN);
      }
    ...
    if (! currentState.isDocked() && isDockingActionAccepted(dae, dswe)){
      ...
      k.setDockableState(DockableState.STATE_DOCKED);
      fireDockingAction(dae, new DockableStateChangeEvent(currentState, newState));
      
      if (dockable instanceof CompoundDockable){
        // V2.1 : also trigger a dockable state change event for compound dockable children
        updateCompoundChildrenState((CompoundDockable) dockable, DockableState.STATE_DOCKED);
      }
...
private void xmlWriteFloating(PrintWriter out) throws IOException {
  ...
  ArrayList floatingDockables = context.getDockablesByState(this, DockableState.STATE_FLOATING);
...
private void xmlWriteFloatingDockable(Dockable dockable, PrintWriter out) throws IOException{
  ...
  + "\" h=\"" + position.getHeight()
  + "\" />");
  DockableState prev = (DockableState) previousFloatingDockableStates.get(dockable);
...
private void xmlWriteTabGroups(PrintWriter out) throws IOException {
  ...
  ArrayList processedDockables = new ArrayList();
  Iterator it = tabbedGroups.keySet().iterator();
  while (it.hasNext()){
    Dockable d = (Dockable) it.next();
    if (! processedDockables.contains(d)){
      processedDockables.add(d);
      LinkedList tabList = (LinkedList) tabbedGroups.get(d);
...
private void xmlBuildAutoHideNode(AutoHideButtonPanel borderPanel, Element dockableElt) throws SAXException {
  ...
  context.setDockableState(dockable, new DockableState(this, dockable, DockableState.STATE_HIDDEN, position));
  dockable.getDockKey().setDockableState(DockableState.STATE_HIDDEN);
  btn.init(dockable, borderPanel.getBorderZone());
  borderPanel.add(btn);
  
  if (dockable instanceof CompoundDockable){
    // check for children and build them
    xmlBuildCompoundDockable((CompoundDockable) dockable, dockableElt, DockableState.STATE_HIDDEN);
  }
...
private void xmlBuildFloatingNode(NodeList dockables, Rectangle bounds) throws SAXException {
  ...
  for (int i=0; i < dockables.getLength(); i++){
    ...
    DockableState state = new DockableState(this, dockable, istate, position);
    ...
    context.setDockableState(dockable, new DockableState(this, dockable, DockableState.STATE_FLOATING, position));
    dockable.getDockKey().setDockableState(DockableState.STATE_FLOATING);
    
    if (dockable instanceof CompoundDockable){
      // check for children and build them
      xmlBuildCompoundDockable((CompoundDockable) dockable, dockableElt, DockableState.STATE_FLOATING);
    }
  }
...
private void xmlBuildDockingPanelNode(Node node) throws SAXException {
  if (node.getNodeType() == Node.ELEMENT_NODE){
    Component comp = xmlCreateComponent((Element) node, DockableState.STATE_DOCKED);
    if (comp != null){
...
private Component xmlCreateComponent(Element elt, int dockableState) throws SAXException {
  if (elt.getNodeName().equals("Split")){
    SplitContainer split = xmlBuildSplitContainer(elt, dockableState);
    return split;
    ...
    .createDockableContainer(d, DockableContainerFactory.PARENT_SPLIT_CONTAINER);
    context.setDockableState(d, new DockableState(this, d, dockableState));
    d.getDockKey().setDockableState(dockableState);
    sdc.installDocking(this);
    
    if (d instanceof CompoundDockable){
      // check for children and build them
      xmlBuildCompoundDockable((CompoundDockable) d, elt, dockableState);
    }
    return (Component) sdc;
  } else if (elt.getNodeName().equals("TabbedDockable")){
    
    TabbedDockableContainer tdc = xmlBuildTabbedDockableContainer(elt, dockableState);
...
private void xmlBuildCompoundDockable(CompoundDockable dockable,
    Element compoundElt, int dockableState) throws SAXException{
  ...
  for (int i = 0, len = children.getLength(); i < len; i++) {
    ...
    if (node.getNodeType() == Node.ELEMENT_NODE){
      ...
      if (elt.getNodeName().equals("Key")){
        ...
        Component comp = xmlCreateComponent(elt, dockableState);
...
private SplitContainer xmlBuildSplitContainer(Node node, int dockableState) throws SAXException{
  Element elt = (Element) node;
  ...
  for (int i = 0; i < node.getChildNodes().getLength(); i++) {
    ...
    if (child.getNodeType() == Node.ELEMENT_NODE){
      Component comp = xmlCreateComponent((Element)child, dockableState);
      if (left){
...
private TabbedDockableContainer xmlBuildTabbedDockableContainer(Element elt,
    int dockableState) throws SAXException {
  ...
  for (int i = 0; i < elt.getChildNodes().getLength(); i++) {
    ...
    if (child.getNodeType() == Node.ELEMENT_NODE){
      ...
      if (d instanceof CompoundDockable){
        ...
        xmlBuildCompoundDockable((CompoundDockable) d, (Element) child, dockableState);
      }
    }
  }
  return tdc;
...
public void addDockable(Dockable dockable, RelativeDockablePosition position){
  ...
  DockableState newState = new DockableState(this, dockable, DockableState.STATE_DOCKED, position);
  
  DockableState currentState = getDockableState(dockable);
  int currentStateAsInt = getStateAsInt(currentState);
  
  DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
  DockingActionEvent dae = new DockingActionAddDockableEvent(this, dockable, currentStateAsInt, newState.getState(), dockingPanel);
  if (isDockingActionAccepted(dae, dswe)){
    SingleDockableContainer sdc = RelativeDockingUtilities.insertDockable(
...
public void replace(Dockable base, Dockable replacer){
  ...
  DockableState newBaseState = new DockableState(replacerState.getDesktop(), base, replacerState.getState());
  DockableState newReplacerState = new DockableState(baseState.getDesktop(), replacer, baseState.getState());
  ...
  if (tdcBase != null){
    ...
    if (sdcBaseParent instanceof SplitContainer){
      ...
      SingleDockableContainer sdc = DockableContainerFactory.getFactory().createDockableContainer(
          replacer, DockableContainerFactory.PARENT_SPLIT_CONTAINER);
      sdc.installDocking(this);
      if (isLeft){
...
private static int getStateAsInt(DockableState state){
  if (state == null){
    return DockableState.STATE_CLOSED;
  } else {
...
public void addDockable(CompoundDockable base, Dockable dockable) {
  ...
  DockingActionEvent dae = new DockingActionAddDockableEvent(this, dockable,
      currentStateAsInt, newState.getState(), panel);
...
public void addHiddenDockable(Dockable dockable, RelativeDockablePosition dockedPosition){
  ...
  DockableState newState = new DockableState(this, dockable, DockableState.STATE_HIDDEN, dockedPosition);
  DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
  DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, dockable, currentStateAsInt,
      DockableState.STATE_HIDDEN);
  DockKey k = dockable.getDockKey();
  
  if (currentStateAsInt != DockableState.STATE_HIDDEN && isDockingActionAccepted(dae, dswe)){
    AutoHideButton btn = (AutoHideButton) autoHideButtons.get(k);
    if (btn == null) {
      btn = new AutoHideButton();
      autoHideButtons.put(k, btn);
...
public void addToTabbedGroup(Dockable base, Dockable newTab){//2005/07/13
  ...
  LinkedList group = (LinkedList) tabbedGroups.get(base);
  if (group == null){
...
public void removeFromTabbedGroup(Dockable dockable){ // 2005/07/13
  /* package protected */
     ...
     *
     */
  LinkedList group = (LinkedList) tabbedGroups.get(dockable);
  if (group != null){
...
private boolean isDockingActionAccepted(DockingActionEvent dae, DockableStateWillChangeEvent dse){
  ...
  if (accepted){
    if (dse != null && dae.getInitialDockableState() != dae.getNextDockableState()){
